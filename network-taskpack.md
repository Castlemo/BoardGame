...
### 5.3 메시지 타입별 데이터 구조
- [ ] `PLAYER_JOIN`: { name, color }
- [ ] `GAME_STATE_UPDATE`: { currentPlayer, turnNumber, board, players }
- [ ] `ROLL_DICE`: { diceSum, dice1, dice2, isDouble }
- [ ] `BUY_CITY`: { tileIndex }
- [ ] 기타 필요한 메시지 데이터 구조 정의
...
### 7.1 GameUI 수정
- [ ] 네트워크 모드 플래그 추가 (`isNetworkMode`, `isHost`)
- [ ] 로컬 플레이어와 원격 플레이어 구분
- [ ] 원격 플레이어 턴일 때 UI 비활성화
- [ ] 서버로부터 상태 업데이트 수신 시 UI 반영

### 7.2 턴 동기화
- [ ] 호스트: 턴 진행 후 모든 클라이언트에 상태 브로드캐스트
- [ ] 클라이언트: 자신의 턴에만 액션 전송, 타인 턴은 대기
- [ ] 주사위 굴리기 결과 동기화
- [ ] 타일 도착 이벤트 동기화

### 7.3 액션 처리
- [ ] 클라이언트 액션 → 서버 전송 → 서버 검증 → 브로드캐스트
- [ ] 구매/업그레이드/인수/패스 액션 동기화
- [ ] 찬스 카드 결과 동기화
- [ ] Phase Delete 동기화

### 7.4 상태 동기화
- [ ] 플레이어 상태: 위치, 현금, 소유 도시, 무인도 턴
- [ ] 보드 상태: 도시 레벨, 소유자, 삭제된 타일
- [ ] 턴 상태: 현재 플레이어, 턴 번호, 더블 여부
- [ ] 동기화 충돌 방지 (서버를 신뢰할 수 있는 소스로 지정)
...
- [ ] 클라이언트: 서버 연결 끊김 시 메인 메뉴로 복귀
- [ ] 모든 플레이어에게 알림

### 8.3 예외 처리
- [ ] 소켓 예외 (IOException, SocketException)
- [ ] JSON 파싱 예외
- [ ] 잘못된 메시지 타입 처리
- [ ] 로그 시스템 추가 (네트워크 디버깅용)
